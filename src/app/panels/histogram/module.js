    ...
    $scope.get_data = function(data, segment, query_id) {
      ...
      if($scope.panel.annotate.enable) {
        ...
        request = request.sort([
          $scope.ejs.Sort($scope.panel.annotate.sort[0]).order($scope.panel.annotate.sort[1]).unmappedType('keyword'),
          $scope.ejs.Sort($scope.panel.time_field).desc().unmappedType('date')
        ]);
      }
          ...
          if (item) {
            if (item.series.info.alias || scope.panel.tooltip.query_as_alias) {
              group = '<small style="font-size:0.9em;">' +
                ...
                kbn.xmlEnt(item.series.info.alias || item.series.info.query)+
              '</small><br>';
            } else {
